...
namespace Dbarone.Template
{
    ...
    public class RegexTemplater
    {
        ...
        private Dictionary<string, object> GetProperties(object model, Dictionary<string, object> properties, string prefix, bool includeRoot)
        {
            if (model != null) {
        ...
        private bool IsIndexedProperty(PropertyInfo pi)
        {
            return (pi.GetIndexParameters().Count() > 0);
        }

        // possibly make this extension method
        private object GetDictionaryElement(Dictionary<string, object> dictionary, string key)
        {
            object ret = null;
        ...
        private string SimpleReplace(string template, Regex pattern, Dictionary<string, object> variables)
        {
